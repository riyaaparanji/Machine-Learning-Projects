...
class ChaseEnvironment:
    ...
    def reset(self):
        ...
        self.target_y = random.randint(self.border_width + 300, self.screen_height - self.border_width - self.target_size - 300)
    ...
    def chaser_action(self, action):
        ...
        # Enable obstacle collision detection
    ...
    def render(self):
        ...
        pygame.display.set_caption("Chase Environment with Obstacles")
        return screen
    ...
    def update_screen(self, screen):
        ...
        obstacles_text = font.render(f'Obstacles: {self.num_obstacles}', True, (0, 0, 0))
        
        screen.blit(score_text, (20, 20))
        screen.blit(speed_text, (20, 50))
        screen.blit(target_speed_text, (20, 80))
        screen.blit(obstacles_text, (20, 110))
...
class QLearningAgent:
    def __init__(self, env, learning_rate=0.1, discount_factor=0.95, exploration_rate=1.0, 
        ...
        self.model_path = "q_learning_chase_model_obstacles.pkl"
    ...
    def discretize_state(self, state):
        ...
        obstacle_proximity = self.obstacle_proximity(cx, cy, obstacles)
        
    
        discrete_state = (angle_bin, distance_bin, target_angle_bin, chaser_speed_bin, obstacle_proximity)
        
        return discrete_state
    
    def obstacle_proximity(self, cx, cy, obstacles):
        ...
        for obstacle in obstacles:
            ox, oy = obstacle.centerx, obstacle.centery
            distance = math.sqrt((ox - cx)**2 + (oy - cy)**2)
            
            # Only consider obstacles within detection range
            if distance <= detection_range:
                # Determine direction from chaser to obstacle
                angle = math.atan2(oy - cy, ox - cx)
                # Convert angle to 0-7 (8 directions)
                direction = int(((angle + math.pi/8) % (2*math.pi)) / (math.pi/4))
                # Set the bit for that direction
                proximity |= (1 << direction)
